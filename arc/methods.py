...
class SplitConformalCC:
    def __init__(self, X, Y, black_box, alpha, random_state=2020, verbose=False):
        ...
        labels = np.sort(np.unique(Y_calib))
        self.thresholds = np.zeros(len(labels))

        for i, label in enumerate(labels):
            idxs = np.argwhere(Y_calib == label)[:, 0]
            X_calib_class = X_calib[idxs]
            Y_calib_class = Y_calib[idxs]

            # Form prediction sets on calibration data
            p_hat_calib = self.black_box.predict_proba(X_calib_class)
            grey_box = ProbAccum(p_hat_calib)

            epsilon = np.random.uniform(low=0.0, high=1.0, size=len(idxs))
            alpha_max = grey_box.calibrate_scores(Y_calib_class, epsilon=epsilon)
            scores = alpha - alpha_max
            level_adjusted = (1.0-alpha)*(1.0+1.0/float(len(idxs)))
            alpha_correction = mquantiles(scores, prob=level_adjusted)
            alpha_calibrated = alpha - alpha_correction
            self.thresholds[i] = alpha_calibrated
    ...
    def predict(self, X, gamma=0):
        ...
        sorted_classes = np.argsort(self.thresholds)
        sorted_threshold = np.sort(self.thresholds)
        cc_threshold = sorted_threshold[gamma]
        S_hat = grey_box.predict_sets(cc_threshold, epsilon=epsilon)
        return S_hat, sorted_classes[gamma:]
